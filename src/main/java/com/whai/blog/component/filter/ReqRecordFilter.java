package com.whai.blog.component.filter;

import cn.hutool.core.date.StopWatch;
import com.whai.blog.config.ReqInfoContext;
import com.whai.blog.service.def.StatisticsSettingService;
import com.whai.blog.utils.def.*;
...
//        reqInfo.setPayload(requestWrapper.getBodyString());
    ...
    final String DEVICE_CONSTANTS = "device";
    ...
    private String getOrInitDeviceId(HttpServletRequest request, HttpServletResponse response) {
        ...
        Cookie device = SessionUtil.findCookieByName(request, DEVICE_CONSTANTS);
        if (device == null) {
            deviceId = UUID.randomUUID().toString();
            if (response != null) {
                response.addCookie(SessionUtil.newCookie(DEVICE_CONSTANTS, deviceId));
            }
            return deviceId;
        }
